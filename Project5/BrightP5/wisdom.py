...
class Fitness:
    ...
    def getDist(self):
        ...
        pathDist+= calcDistance(self.path[int(len(self.path))-1],self.path[0])
        self.dist = pathDist
        return self.dist
    def getFitness(self):
...
def breed(p1,p2):
    ...
    g1 = int(random.randrange(0,len(cities)))
    g2 = int(random.randrange(0,len(cities)))
...
def generations(currentGen,mutRate,genNum):
    ...
    mutatePop = bredPop
    if best[0].getDist == distOverTime[genNum-1]:
...
def formatAndShow(out):
    ...
    for c in out: 
        formatx.append(c.x)
        formaty.append(c.y)    
    formatx.append(out[0].x)
    formaty.append(out[0].y)
...
def main(argv):
    ...
    for x in range(0,numberOfGens):
        ...
        combindedPop = []
        for y in range(0,10):
            print("gen: {0} subgen: {1}".format(x+1,y+1))
            population = generations(population,.05,x+y)
            pops.append(population)
        for pop in pops:
            for y in range(0,int(len(pop)/4)):
                combindedPop.append(pop[y])
        #combindedPop = list(set(combindedPop))
        #noDupPop = [city for city in initialPopulation if city not in combindedPop]
        population = combindedPop
        global repeatCount
        if(repeatCount == 2):
            break
